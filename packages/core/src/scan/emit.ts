...
/** A source file becomes a specifier a bundler can trace: relative, and `.js`. */
function specifierOf(filePath: string, outFile: string): string {
  const rel = relative(dirname(outFile), filePath).replace(/\.tsx?$/, '.js');
