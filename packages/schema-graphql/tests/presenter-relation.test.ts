...
function fakeApp(presenterFields: string[]) {
  const facade = {
    list: async () => [],
  ...
  return {
    fronds: [{
      ...
      presenters: [{ entityName: 'order', fields: presenterFields }],
    }],
...
function build(presenterFields: string[]) {
  const builder = new SchemaBuilder({});
  builder.queryType({});
  builder.mutationType({});
  registerAll(builder, fakeApp(presenterFields));
