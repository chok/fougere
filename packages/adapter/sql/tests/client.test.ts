...
describe('storage.client', () => {
  it("rend le client que le storage enveloppe", async () => {
    const { storage } = await app();
    expect((storage as { client?: unknown }).client).toBeDefined();
  });

  it('atteint les mêmes lignes que le port', async () => {
    const { storage } = await app();
    await storage.create({ name: 'Clavier', price_cents: 4990 });

    const client = (storage as { client: any }).client;
  ...
  it("reste la porte de ce que le port n'offre pas", async () => {
    const { storage } = await app();
    const client = (storage as { client: any }).client;
    await storage.create({ name: 'A', price_cents: 100 });
    await storage.create({ name: 'B', price_cents: 300 });
