/**
 * The storage against a real SQLite database — not a mock.
 *
...
const seed = (title: string) => storage.create({ title, body: 'b', secret: 's' });
...
describe('findById / findBy / findAllBy', () => {
  it('reads back by primary key', async () => {
    ...
    expect((await storage.findById(id)).title).toBe('Hello');
  });

  it('returns undefined for a missing row', async () => {
    expect(await storage.findById('nope')).toBeUndefined();
  });

  it('finds by criteria', async () => {
    await seed('A');
    await seed('B');
    expect((await storage.findBy({ title: 'B' })).title).toBe('B');
    expect(await storage.findAllBy({ body: 'b' })).toHaveLength(2);
  });
});
...
describe('update', () => {
  it('writes and returns the complete row', async () => {
    const { id } = await seed('Hello');
    const updatedRow = await storage.update(id, { title: 'Changed' });
  ...
  it('stamps updatedAt even when absent from the input', async () => {
    const { id } = await seed('Hello');
    const before = (await storage.findById(id)).updatedAt;
    await new Promise((r) => setTimeout(r, 5));
    const after = (await storage.update(id, { title: 'Changed' })).updatedAt;
...
describe('delete', () => {
  it('removes the row and reports it', async () => {
    ...
    expect(await storage.findById(id)).toBeUndefined();
  });
...
describe('list', () => {
  ...
  it('orders by a field', async () => {
    const desc = await storage.list({ orderBy: 'title', order: 'desc' });
    expect(desc.map((p: any) => p.title)).toEqual(['C', 'B', 'A']);
  });

  it('counts on demand', async () => {
    expect((await storage.list({ count: true })).total).toBe(3);
  });

  it('counts what the filter matches, not what the table holds', async () => {
    await storage.update((await storage.list({ orderBy: 'title' }))[0].id, { published: true });

    const page = await storage.list({ where: { published: true }, count: true });
  ...
  it('counts the whole filter, not the page', async () => {
    ...
    const page = await storage.list({ limit: 2, count: true });
    expect(page).toHaveLength(2);
    expect(page.total).toBe(3);
  });

  it('offsets', async () => {
    expect(await storage.list({ offset: 2 })).toHaveLength(1);
  });
});
...
describe('output()', () => {
  it('restricts the fields a read hands back', async () => {
    const { id } = await seed('Hello');
    expect(await storage.findById(id)).toHaveProperty('secret');

    const scoped = await storage.output(PostPublic).findById(id);
  ...
  it('restricts on list too, keeping pagination metadata', async () => {
    await seed('B');
    await seed('C');
    const rows = await storage.output(PostPublic).list({ limit: 1 });
    expect(rows[0]).not.toHaveProperty('secret');
    expect(rows.hasMore).toBe(true);
  });

  it('leaves the unscoped storage untouched', async () => {
    const { id } = await seed('Hello');
    storage.output(PostPublic);
    expect(await storage.findById(id)).toHaveProperty('secret');
  });
});
...
describe('a criterion naming a set', () => {
  it('reads them all in one query — the batch form a relation needs', async () => {
    const a = await storage.create({ title: 'one', body: 'a', secret: 's' });
    await storage.create({ title: 'two', body: 'b', secret: 's' });
    const c = await storage.create({ title: 'three', body: 'c', secret: 's' });

    const rows = await storage.findAllBy({ id: [a.id, c.id] });
    expect(rows.map((r: any) => r.title).sort()).toEqual(['one', 'three']);
  });

  it('an empty set matches nothing — never the whole table', async () => {
    await storage.create({ title: 'one', body: 'a', secret: 's' });
    expect(await storage.findAllBy({ id: [] })).toEqual([]);
  });

  it('a repeated value is asked once, and one row comes back', async () => {
    const a = await storage.create({ title: 'one', body: 'a', secret: 's' });
    expect(await storage.findAllBy({ id: [a.id, a.id] })).toHaveLength(1);
  });

  it('values cross the codec, as a single criterion already does', async () => {
    const a = await storage.create({ title: 'one', body: 'a', secret: 's' });
    expect(await storage.findAllBy({ id: [a.id] })).toEqual([await storage.findById(a.id)]);
  });
});
...
describe('findByKeys', () => {
  it('answers a map keyed by the primary key — a page zips by key, not by position', async () => {
    await storage.create({ title: 'one', body: 'a', secret: 's' });
    const b = await storage.create({ title: 'two', body: 'b', secret: 's' });
    const c = await storage.create({ title: 'three', body: 'c', secret: 's' });

    const found = await storage.findByKeys([c.id, b.id]);
  ...
  it('a miss is an absent key — not a hole, not an error', async () => {
    ...
    const found = await storage.findByKeys([a.id, 'never-created']);
    expect(found.size).toBe(1);
    expect(found.has('never-created')).toBe(false);
    expect((await storage.findByKeys([])).size).toBe(0);
  });

  it('a repeated key is queried once and is one entry — a map cannot hold it twice', async () => {
    const a = await storage.create({ title: 'one', body: 'a', secret: 's' });
    const found = await storage.findByKeys([a.id, a.id]);
    expect(found.size).toBe(1);
  ...
  it('values cross the codec, as they do on every other read path', async () => {
    const a = await storage.create({ title: 'one', body: 'a', secret: 's' });
    const found = await storage.findByKeys([a.id]);
    // A date read through here must be a Date, not the column's string.
    expect(found.get(a.id)).toEqual(await storage.findById(a.id));
  });
});
...
describe('findAllByKeys — the other direction of a relation', () => {
  it('groups by the key each row points at, in one query', async () => {
    await storage.create({ title: 'a1', body: 'shared', secret: 's' });
    await storage.create({ title: 'a2', body: 'shared', secret: 's' });
    await storage.create({ title: 'b1', body: 'other', secret: 's' });

    const grouped = await storage.findAllByKeys('body', ['shared', 'other']);
  ...
  it('a key with no row is absent — never an empty array to be told apart', async () => {
    await storage.create({ title: 'a1', body: 'shared', secret: 's' });
    const grouped = await storage.findAllByKeys('body', ['shared', 'nobody']);
    expect(grouped.has('nobody')).toBe(false);
    expect((await storage.findAllByKeys('body', [])).size).toBe(0);
  });

  it('asks nothing outside the keys — the whole table is never the answer', async () => {
    await storage.create({ title: 'a1', body: 'shared', secret: 's' });
    await storage.create({ title: 'z', body: 'unasked', secret: 's' });
    const grouped = await storage.findAllByKeys('body', ['shared']);
...
describe('a key set larger than one statement can bind', () => {
  ...
  beforeEach(async () => {
    ...
    for (let i = 0; i < 50; i++) await storage.create({ title: `t${i}`, body: 'b', secret: 's' });
  });

  it('reads them all — sliced, and the caller never learns there were several', async () => {
    const real = (await storage.list()).map((r: any) => r.id);
    const keys = [...real, ...Array.from({ length: MANY }, (_, i) => `absent-${i}`)];

    const found = await storage.findByKeys(keys);
  ...
  it('groups them all on the dual, slices concatenated', async () => {
    ...
    const grouped = await storage.findAllByKeys('body', keys);
  ...
  it('refuses on `list`, naming the gesture that handles it', async () => {
    ...
    await expect(storage.list({ where: { id: keys } }))
  ...
  it('refuses two oversized criteria rather than guessing their cross product', async () => {
    const keys = Array.from({ length: MANY }, (_, i) => `k${i}`);
    await expect(storage.findAllBy({ id: keys, title: keys }))
...
describe('upsert', () => {
  it('writes when the row is absent, exactly like create', async () => {
    const row = await storage.upsert({ id: 'p1', title: 'first', body: 'b', secret: 's' });
  ...
  it('replaces when it is there — the second run of an import must not throw', async () => {
    await storage.upsert({ id: 'p1', title: 'first', body: 'b', secret: 's' });
    const again = await storage.upsert({ id: 'p1', title: 'revised', body: 'b2', secret: 's' });
  ...
  it('generates the key when the caller brings none', async () => {
    const row = await storage.upsert({ title: 'no key of mine', body: 'b', secret: 's' });
...
describe('upsertAll', () => {
  it('writes a page in one statement, and says how many', async () => {
    ...
    expect(await storage.upsertAll(page)).toBe(200);
    expect(await storage.list()).toHaveLength(200);
  });
  ...
  it('replaces on a second pass — the shape of a re-import', async () => {
    ...
    await storage.upsertAll(page);
    await storage.upsertAll(page.map((r) => ({ ...r, title: `${r.title} révisé` })));

    expect(await storage.list()).toHaveLength(50);
    expect((await storage.findById('p0')).title).toBe('t0 révisé');
  });
  ...
  it('slices by rows × COLUMNS — a statement binds values, not rows', async () => {
    ...
    expect(await storage.upsertAll(many)).toBe(8_000);
    expect(await storage.list()).toHaveLength(8_000);
  });

  it('keeps the moment each row appeared', async () => {
    await storage.upsertAll([{ id: 'p1', title: 'first', body: 'b', secret: 's' }]);
    const first = await storage.findById('p1');
    await new Promise((r) => setTimeout(r, 5));
    await storage.upsertAll([{ id: 'p1', title: 'revised', body: 'b', secret: 's' }]);
    const again = await storage.findById('p1');
