...
import { typeSourceOf, facadeTypeSourceOf, type SchemaDescriptor } from '@fougere/schema';
...
export default class SyncHandler {
  ...
  async execute(input: { name: string; from: string }): Promise<{ path: string; entities: string[] }> {
    ...
    for (const { name, schema: descriptor, ops } of target.entities) {
      ...
      /**
       * Two readings of one card, in one file.
       *
       * `reconstruct` gives the JUDGE — validate, from, getFields — and returns
       * `SchemaConstructor<Fields>`, a generic. So until now a synced entity validated
       * perfectly at runtime and taught the compiler nothing: `post.title` did not
       * exist. `typeSourceOf` is the second reading, and the declaration finally
       * travels to the type system too.
       *
       * The interface and the const share a name on purpose — that is how a class
       * behaves, so `import { Post }` hands over both the value and the type, and
       * `useQuery<Post>(Post, 'list')` needs one import rather than two.
       */
      const code = [
        `import { reconstruct } from '@fougere/schema';`,
        ``,
        `// Generated by \`fougere sync\` from ${baseUrl} — do not edit.`,
        typeSourceOf(descriptor as SchemaDescriptor, { name: className }),
        ``,
        `export const ${className} = reconstruct(${cardJson});`,
    ...
    // Barrel index
    // The value AND the type — a consumer writing `import { Post }` gets a judge it
    // can call and a shape it can annotate with.
    const indexLines = entityNames.flatMap((name) => [
      `export { default as ${name}, type ${name} } from './entities/${name}.js';`,
