...
describe('validation — edge cases', () => {
  it('coerces a date-string to a Date in the output', () => {
    const r = InputValidator.of({ when: date() }).validate({ when: '2026-05-29T10:00:00Z' });
    expect(r.success).toBe(true);
    if (r.success) expect(r.data.when).toBeInstanceOf(Date);
  });

  it('rejects an invalid date-string', () => {
    expect(InputValidator.of({ when: date() }).validate({ when: 'not-a-date' }).success).toBe(false);
  });

  it('passes a Date instance through unchanged', () => {
    const d = new Date();
    const r = InputValidator.of({ when: date() }).validate({ when: d });
    expect(r.success).toBe(true);
    if (r.success) expect(r.data.when).toBe(d);
  });

  it('rejects NaN for a number field', () => {
    expect(InputValidator.of({ n: number() }).validate({ n: NaN }).success).toBe(false);
  });
  ...
  it('treats empty string as present (fails min, passes without min)', () => {
    expect(InputValidator.of({ t: text({ min: 1 }) }).validate({ t: '' }).success).toBe(false);
    expect(InputValidator.of({ t: text() }).validate({ t: '' }).success).toBe(true);
  });
  ...
  it("updated(): absence is legal in create and patch — storage stamps, not validation", () => {
    ...
    const create = InputValidator.of(fields).validate({ title: 'x' });
    expect(create.success).toBe(true);
    if (create.success) expect('updatedAt' in create.data).toBe(false);
    const patch = InputValidator.of(fields, { patch: true }).validate({ title: 'x' });
  ...
  it('readOnly(oneOf) — a server-owned enum: supplied in create or patch is an error', () => {
    ...
    expect(InputValidator.of(fields).validate({}).success).toBe(true);
    const create = InputValidator.of(fields).validate({ status: 'published' });
    expect(create.success).toBe(false);
    const patch = InputValidator.of(fields, { patch: true }).validate({ status: 'published' });
    expect(patch.success).toBe(false);
  });
