...
describe('validation — edge cases', () => {
  it('coerces a date-string to a Date in the output', () => {
    const r = RowJudge.of({ when: date() }).validate({ when: '2026-05-29T10:00:00Z' });
    expect(r.success).toBe(true);
    if (r.success) expect(r.data.when).toBeInstanceOf(Date);
  });

  it('rejects an invalid date-string', () => {
    expect(RowJudge.of({ when: date() }).validate({ when: 'not-a-date' }).success).toBe(false);
  });

  it('passes a Date instance through unchanged', () => {
    const d = new Date();
    const r = RowJudge.of({ when: date() }).validate({ when: d });
    expect(r.success).toBe(true);
    if (r.success) expect(r.data.when).toBe(d);
  });

  it('rejects NaN for a number field', () => {
    expect(RowJudge.of({ n: number() }).validate({ n: NaN }).success).toBe(false);
  });
  ...
  it('treats empty string as present (fails min, passes without min)', () => {
    expect(RowJudge.of({ t: text({ min: 1 }) }).validate({ t: '' }).success).toBe(false);
    expect(RowJudge.of({ t: text() }).validate({ t: '' }).success).toBe(true);
  });
  ...
  it("updated(): absence is legal in create and patch — storage stamps, not validation", () => {
    ...
    const create = RowJudge.of(fields).validate({ title: 'x' });
    expect(create.success).toBe(true);
    if (create.success) expect('updatedAt' in create.data).toBe(false);
    const patch = RowJudge.of(fields, { patch: true }).validate({ title: 'x' });
  ...
  it("immutable(): re-supplied in a patch → 'Immutable'; create accepts; absent patch untouched", () => {
    ...
    expect(RowJudge.of(fields).validate({ slug: 'a', title: 'x' }).success).toBe(true); // create
  ...
  it('readOnly(oneOf) — a server-owned enum: supplied in create or patch is an error', () => {
    ...
    expect(RowJudge.of(fields).validate({}).success).toBe(true);
    const create = RowJudge.of(fields).validate({ status: 'published' });
    expect(create.success).toBe(false);
    const patch = RowJudge.of(fields, { patch: true }).validate({ status: 'published' });
    expect(patch.success).toBe(false);
  });
  ...
  it("unknown key → 'Unknown field' — refused, never stripped, create and patch alike", () => {
    ...
    const create = createJudge.validate({ title: 'x', status: 'published' });
    expect(create.success).toBe(false);
    if (!create.success) expect(create.errors[0]).toEqual({ path: 'status', message: 'Unknown field' });
    const patch = RowJudge.of(fields, { patch: true }).validate({ titel: 'typo' });
    expect(patch.success).toBe(false);
    if (!patch.success) expect(patch.errors[0]).toEqual({ path: 'titel', message: 'Unknown field' });
    expect(createJudge.validate({ title: 'x' }).success).toBe(true);
  });
});
