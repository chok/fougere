...
const columns = 'id, title, body, excerpt, created_at AS "createdAt", published_at AS "publishedAt"';
...
export const create = api({ expose: true, method: 'POST', path: '/posts' }, async (input: CreatePost): Promise<Post> => {
  return (await db.rawQueryRow<Post>(
    `INSERT INTO posts (id, title, body, excerpt) VALUES ($1, $2, $3, $4) RETURNING ${columns}`,
    randomUUID(), input.title, input.body, input.excerpt ?? null,
  ))!;
});
