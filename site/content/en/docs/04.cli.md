...
where the other half of a call can be watched. Without it, the default is
`http://127.0.0.1:3000`.

An app that never installed the package answers `Unknown rpc operation 'calls'. It serves
discover.`, and that address is reported as a refusal rather than as silence.
