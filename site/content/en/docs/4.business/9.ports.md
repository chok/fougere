...
held once for the process and **consulted at every emission** rather than copied into each
instance. So nothing is rebuilt when it moves — the handler keeps the very object it was
handed.
...
// re-reading later, in the host that owns the process
...
When a key was CONSUMED — `db` opened a connection, `ports:` registered a class — the
value cannot move under what it built, so the thing is built again: `reloadFougere()`
instantiates the app and lets the previous one go. A call already running finishes on the
old app, because it is **drained** first:
...
`drain(timeoutMs)` REJECTS on its deadline naming what is left, rather than resolving as
if it had worked — its caller is about to close a connection under whatever remains.

`applyConfig` is the one place that answers *what a re-read changes in a running app*. A
key is consulted when this function does something with it; everything else was consumed
to build something and is reported instead of silently ignored. Fougere catches no signal
of its own: a process belongs to its host.
