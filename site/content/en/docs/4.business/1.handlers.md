...
**There is no primitive for this yet.** A Frond that wants to say *a post was published*
and let whoever cares react has to name each interested party and call them, which means it
carries the list of everything that follows from its own action.

An `EventBus` used to sit here. It resolved by **string** — the one place in the framework
that did not resolve by type, so a misspelled event name raised nothing — carried an
`unknown` payload, and awaited every listener, handing their failures back to the emitter.
It was removed rather than published, because a primitive that contradicts the framework's
central rule is worse than an acknowledged gap.

What every call in Fougere has in common is **one recipient**: `remotes` names one address
per Frond, `Facade<T>` resolves one door. One fact reaching several readers is the other
half, and it is designed, not shipped.
