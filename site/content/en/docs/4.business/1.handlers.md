...
The container also registers an `EventBus` singleton — an in-process pub/sub:
...
Two caveats. It resolves by **string**, the one place in the framework that does not
resolve by type — a misspelled event name raises nothing. And it is **in-process**: after a
split, an emitter and a listener that no longer share a process no longer see each other.
Nothing subscribes to it by default.
