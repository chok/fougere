...
Every other call names **one** recipient: `fronds:` one address per Frond,
[`Facade<T>`](/docs/business/handlers#reaching-another-frond) one facade. `Emit<T>` names a
...
A subscriber is an ordinary operation. It keeps its facade, its validator and its middlewares,
because an emission and a direct call **are the same call** — which is also why it appears
in your public surface. Put it under a named surface — `handlers/<surface>/` — to keep it
out of a client one.

### It is a resolver, not a channel

A bus moves messages: a queue, an envelope, a delivery semantic. This has none of that. It
answers *who*, then hands over to the facade that already exists — `resolve` returning N
...
- **Nothing is durable, and nothing here will be.** Kill the listener's process and the
  ...
  where a broker of under a hundred lines holds the queue and the Frond holds none of it.

### Across processes

`fronds:` is the only line that changes. The listener's Frond stays on disk either way: it
is scanned, so the emitter knows its signature; declaring it remote only says it is not
hosted here, and its facade resolves to a stand-in that sends the call.
...
Beyond two processes, [emit-fleet](https://github.com/chok/fougere/tree/main/demos/emit-fleet)
is a hub and N devices over one socket: facts travel in both directions, a device that
starts before the hub reconnects on its own, and addressing **one** device is done in the
data — because `fronds:` names a kind of Frond, never an instance of one.

### Across repositories

Two things stop at a repository boundary, and only one of them is `fronds:`.
...
// in — the same local dispatch, validator and middlewares included
...
validated against the emitter's own declaration.

A carrier has no address, and that is the line between the three words. `Emit` announces a
subject and `Fact` receives one, so neither names anybody — a carrier can take them across.
A `Pipe<T>` — the op that FINISHES a fact — is CALLED: it has an address, local or named in
`fronds:`, and
it answers. So a link crosses a process and never a carrier, which is the same line `Ask`
draws.
...
A fact meets the **same validator** as any other input: a key outside the contract is
