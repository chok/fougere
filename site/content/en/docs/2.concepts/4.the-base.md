...
(`services/` or `repositories/`). As a storage holder, it is handed the `EntityOrm` its
...
resolves even when no repository file exists. Injecting `EntityOrm<E>` directly into a
