...
the menu on the next load, because the card grew a facade — not because a file was written.

## What each screen is read from

| Screen | Read from | Function |
|---|---|---|
| The menu | `card.fronds[].facades[]` | a facade with no schema is not furniture |
...
| The buttons | `facade.ops` | no `create` op, no create button |
...
```ts
// fougere.config.ts
fronds: { billing: 'https://billing.internal' }
```

The `invoice` facade moves to another process, the card still announces it, the same build
...
## The one page that is not a facade

Every screen above renders a facade. **Topology** renders the app:

```
catalog  here               1 entity · 1 facade      → blog (12)
blog     elsewhere          its shape is published by the process that owns it   ← catalog (12)
search   declared, unheard  declared at search.internal:4500 — nothing has answered from it
```
...
which answered from somewhere else, and every call path observed between them. None of that
is declared: a Frond is *elsewhere* because it **answered**, never because `fronds:` said so.

The last line is the one the counted half cannot hold. A Frond that never answered is absent
from it, so the page would show a smaller healthy system exactly when a node is down — it
comes from the report's `declared` half, which counts nothing and is never mixed with the
rest. An unheard Frond is not an opaque one: opaque means it answered and keeps its shape to
itself.
...
- **Remove a resource or an operation.** Hiding a facade in the browser while the façade
  still serves it reads as a permission and enforces nothing. What a facade serves to one
...
- **Write outside the façade.** It runs in a browser, so it can only reach the call
  endpoint — validates, presenters and middlewares are on the path of every read and write it
  makes. This is a consequence, not a precaution.
- **Aggregate.** Storage's only aggregate is `count`, so the dashboard counts rows and
