...
Edit mode composes with a query — the door is aliased because the row is already `post`:
...
1. **Local validation** — `Entity.validate(payload)` runs in the browser with the same
   ...
   `submit()` returns `null`.
2. **Command** — the payload is sent through the door the entity names. On success, active
