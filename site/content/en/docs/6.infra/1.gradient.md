...
## `remotes` is the pull half

The line above says where this app **calls out to**. It does not say where it **pushes
to** — and nothing else does either.

That is not an omission in the page, it is the shape of the framework today. Every
initiator in Fougere names its counterpart:

| what the initiator names | consequence |
|---|---|
| **an interlocutor** — `Facade<PostHandler>`, `remotes`, `useQuery` | necessarily **one** |
| **a subject** — a fact that happened, whoever cares | the number of readers is not its business |

Fougere has the first column. It has no second. One fact reaching several readers — a post
is published, so search reindexes, the newsletter queues, analytics counts — is written
today by naming all three at the emitting site, which means the emitter carries the list
of everything that follows from its own action.

Naming the half that exists is the point of this section: `remotes` reads like *the*
topology statement, and it is one of two. The other is designed and not shipped —
declaring it here is what keeps the gap visible rather than surprising.
