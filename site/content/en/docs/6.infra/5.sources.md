...
`file` (one JSON per row, a directory per entity) and `memory` (a Map, which is also the
fallback for an app with no `db`).

`sql` opens SQLite from a `path`, and the three other engines from a `url`. The driver is the
app's to install, beside it — `better-sqlite3`, `pg`, `mysql2`, or `tedious` and `tarn` — and it
is loaded when a source opens, so an app that opens none installs nothing native. A missing one
is refused with the command that adds it:
...
Any other Kysely dialect — D1, libSQL, PGlite, Neon — is another driver for one of these four
engines, and its connection is not a url: build it in code, and hand it in with
...
Every attach is `READ_ONLY`, and it is the engine that enforces it. This facade reads across
