...
`inputFields` for what may come in, `Judge.onAbsent` for what is required,
...
make the list a second judge to keep in step by hand.

`checkOutput` costs nothing to state — `outputFields` already answers it, so a field the
...
ours is *which* fields belong in a body — `inputFields`, the one reader of the boundary
...
On facts the line says the order out loud, because a fact is judged strictly: a reader
...
Membership comes from `inputFields`, requiredness from the lifecycle axis, and the bounds
