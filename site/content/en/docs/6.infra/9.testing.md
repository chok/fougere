...
applies to topology: `fronds:` sits outside the frond, and the same code runs in-process
...
The five CRUD operations, reads and writes. A write creates a different row at each facade,
...
where facades diverge most, and where each is most tempted to answer in its own words.

Each facade's envelope is taken off before comparing: they wrap differently by construction,
...
hand-written cases through all four facades. Fougere uses it to pin optional-input semantics:
...
Its dual needed nothing at all — `app.deliver` is already the carrier's facade, and it waits
...
straight to the facade is refused by the validator. *Local validator = remote validator* is watched here
...
::demo{name="test-gradient"}
::

## A frozen clock

`created()`, `updated()` and `create: 'now'` are realized in one place —
`applyCreate`/`applyUpdate` — which reads the system clock at the write. Freezing time the way
any project does is therefore enough:
...
`toFake: ['Date']` leaves `setTimeout` and promises running: only the dates hold still.
...
writeFileSync('load.js', loadScript(app, { facade: 'http://127.0.0.1:3000/_fougere/call' }));
await app.dispose();
```
...
A performance test asserts a number of milliseconds and breaks on someone else's laptop.
These two ask about structure instead, and structure does not change with the machine.

```ts
const ran = await statementsOf(() => facade.list());
expect(ran).toHaveLength(1);          // not one per row
```

`statementsOf` returns every statement the block ran, in order. `testApp` boots on real
SQLite, so this is what the handler actually asked the database — and one query per row is
the shape of an N+1. The façade hands a presenter the whole **page** precisely so a lookup
can be done once; nothing refuses `Promise.all(rows.map(…))` inside the field body, and this
is what notices.

Its dual reads the call itself:

```ts
const [op] = await spansOf(app, () => facade.checkout());
expect(op.selfMs).toBeLessThan(0.1 * op.ms);   // it delegates, it does not work
```

`selfMs` is the duration minus what ran under it, so the assertion states that an op
delegates rather than works. A ratio survives a slow machine; a threshold in milliseconds
tests the machine.

::note
`statementsOf` needs `@fougere/adapter-sql` and `spansOf` needs `@fougere/observability`.
Both **refuse** when it is missing rather than returning nothing: zero is what a passing
assertion looks like, and a test that cannot fail is worse than no test.
::
