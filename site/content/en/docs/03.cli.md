...
> `serve()` binds `127.0.0.1` and refuses any other interface: a receiver reads identity
> off the wire, so the loopback bind is what protects it.
...
The rebuilt class validates locally, without an additional request to the host. The
command also writes `.fougere/remotes.json`, which the Nuxt module reads to alias
