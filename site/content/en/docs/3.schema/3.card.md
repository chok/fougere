...
`~standard` exposes the Standard Schema validation interface. The card provides its
serializable description for exchanges across processes or languages.
...
`facts` is the same on every [surface](/docs/business/surfaces) — a door has an audience, a
fact does not.
