...
The two are duals. [`~standard`](/docs/schema/standard-schema) hands the **live** validator to
...
const descriptor = Card.fromSchema(Post).descriptor;       // schema to JSON document
const Rebuilt = Card.fromDescriptor(descriptor).toSchema(); // document to living schema
```

`Card.fromSchema` produces the canonical document used by adapters. `Card.toSchema`
rebuilds the schema at the consumer, which can then validate data locally.

State the shape of a row and the rebuilt schema is a class in the full sense — one name
carrying the validator and the type of what it validates, exactly like `class Post extends
...
### A view says what it is a view of

The card of a [view](/docs/schema/views) carries one extra key, absent from a declaration
...
`from` names the origin and `survived` says what the cut left, keyed by the origin's field
names — a dropped field is present and null. `title` already carried the origin's name, and
carried it alone: two views of one entity described identically, and nothing in the card
said which served which fields. A reader can now answer *a Post without `body`*, and
`fougere sync` can write the derivation rather than a flattened shape.
...
(`[["slug"]]`). In memory a lone `unique()` does not yet know its key; `Card.fromSchema` resolves
...
`Bundle.fromSchemas` produces a self-contained `$defs` document. Each entity uses its lowercased
...
two references instead of recursive nesting. `Card.toSchema` keeps the names;
`Bundle.toSchemas` resolves them to the target entities expected by adapters.

## Serialization limits

A card can contain only serializable data:

- **a relation becomes a name.** `Bundle.toSchemas` can resolve the `() => Author` thunk
  within a bundle.
...
**facade** with no schema stores nothing — a health check, a computation, a search across
several shapes. A **fact** with no schema announces a type the host does not store.

### Why facts are a separate list

Hosting means answering: an entity with no façade is deliberately absent from `facades`, or
the card would publish the shape of tables nobody can reach — the auth tables, typically.

A [fact](/docs/business/facts) is the opposite case. It carries no operation either, but
...
the contract, `fronds:` gives only the location, and across two repositories there is no
colocation. The subscriber had to hand-copy the emitter's declaration.

`facts` is the same on every [surface](/docs/infra/surfaces) — a facade has an audience, a
fact does not.

The doublure router reads `facades` only, and uses them to associate entities with the
addresses declared in `fronds:`. A fact is not routable: nobody calls it, it arrives.
