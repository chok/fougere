...
Le mode édition se compose avec une query — la porte est aliasée parce que la ligne s'appelle déjà `post` :
...
## Le navigateur valide en premier, et gratuitement
...
C'est une projection, pas une seconde règle : le validateur lit la même forme. Un formulaire qui
ignore `attrs` obtient le même verdict, seulement plus tard — et un lecteur d'écran ne
l'obtient jamais.

Trois attributs sont délibérément absents, chacun là où HTML dirait autre chose que la
déclaration :

| Champ | Absent | Parce que |
|---|---|---|
| `date()` | `type` | ni `date` ni `datetime-local` ne produit la chaîne RFC 3339 qu'attend le validateur |
...
2. **Command** — le payload est envoyé par la porte que l'entité nomme. En cas de succès, les
