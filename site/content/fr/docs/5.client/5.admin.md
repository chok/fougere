...
|---|---|---|
| Le menu | `card.fronds[].facades[]` | une porte sans schéma n'est pas du mobilier |
...
| Les boutons | `facade.ops` | pas d'op `create`, pas de bouton créer |
...
search   déclaré, muet       déclaré sur search.internal:4500 — rien n'en a jamais répondu
```
...
entre les deux. Rien de tout ça n'est déclaré : un Frond est *ailleurs* parce qu'il a
**répondu**, jamais parce que `fronds:` l'a dit.

La dernière ligne est celle que la moitié comptée ne peut pas tenir. Un Frond qui n'a jamais
répondu en est absent, donc la page montrerait un système plus petit en bonne santé
exactement quand un nœud est tombé — elle vient de la moitié `declared` du rapport, qui ne
compte rien et n'est jamais mélangée au reste. Un Frond muet n'est pas un Frond opaque :
opaque veut dire qu'il a répondu et garde sa forme pour lui.

Cette op est servie par [`@fougere/observability`](/docs/infra/observability), donc un
...
- **Écrire hors de la façade.** Il tourne dans un navigateur, donc il ne peut atteindre que
  le point d'appel — validateurs, presenters et middlewares sont sur le chemin de chaque lecture
  et de chaque écriture qu'il fait. C'est une conséquence, pas une précaution.
- **Agréger.** Le seul agrégat du port Storage est `count`, donc le tableau de bord compte des
