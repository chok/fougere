...
description: Storage — les gestes que tout stockage répond, pourquoi un ensemble se lit en une requête, et la trappe de sortie qui éteint le validateur.
---
...
## Toute écriture par le port est validée
...
::warning
**Tous les validateurs sont posés sur les méthodes du port.** Une instruction émise par `client`
...
validateur ne tourne pas.
::

Pour « débiter A et créditer B, ou ni l'un ni l'autre », prendre
[`Together`](/fr/docs/business/together) : le même port, validé de la même façon, avec un bloc
pour unité au lieu d'une instruction.

## Se mettre devant

`Storage` est une classe autant qu'une interface, donc on peut poser la sienne devant —
audit, cache, une lecture qui répond d'ailleurs. Un maillon l'étend et en demande une :
...
Il couvre toutes les entités de son propre frond et n'écrit que les gestes qu'il change.
C'est une [chaîne de port](/fr/docs/business/ports), déclarée comme n'importe quelle autre.
