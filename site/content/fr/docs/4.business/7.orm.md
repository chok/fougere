---
title: Le port ORM
description: EntityOrm — les gestes que tout stockage répond, pourquoi un ensemble se lit en une requête, et la trappe de sortie qui éteint le juge.
---

# Le port ORM

`EntityOrm<Post>` est ce qu'un handler reçoit pour atteindre les lignes de Post. C'est un
**port** : un jeu de gestes fixe, sans parfum de domaine, implémenté par le stockage que
l'application a résolu. Vous l'obtenez par type, comme tout le reste :
...
Un handler [`Crud(Post)`](/fr/docs/business/handlers) en tient déjà un. Une requête qui
mérite un nom vit sur un
[repository](/fr/docs/business/handlers#où-vit-une-requête-nommée), pas épelée sur le site
d'appel.
...
| `output(view)` | un ORM restreint aux champs d'une [vue](/fr/docs/schema/views) |
...
n'en rencontre aucun : une valeur que l'entité refuse atterrit dans la table sans un mot.

Il n'a pas non plus d'unité de travail. Il n'y a aucune transaction dans le framework
aujourd'hui, donc « débiter A et créditer B, ou ni l'un ni l'autre » a une seule réponse —
`client` — et c'est exactement là que la garantie est éteinte. La portée reste honnête
(`productOrm.client` atteint les produits, pas toute la base), mais le juge ne tourne pas.
::
