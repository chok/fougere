...
Quand une clé a été CONSOMMÉE — `db` a ouvert une connexion, `ports:` a enregistré une
classe — la valeur ne peut pas bouger sous ce qu'elle a construit, donc la chose est
reconstruite : `reloadFougere()` réinstancie l'app et laisse partir la précédente. Un appel
déjà en cours finit sur l'ancienne, parce qu'elle est **drainée** d'abord :
...
`drain(timeoutMs)` REFUSE à l'échéance en nommant ce qui reste, plutôt que de répondre
comme si ça avait marché — son appelant s'apprête à fermer une connexion sous ce qui reste.
