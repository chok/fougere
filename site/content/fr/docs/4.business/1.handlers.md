...
Dans un handler `Crud(Entity)`, le `this.orm` hérité est alimenté par le repository de
l'entité ; n'injectez pas vous-même `EntityOrm`. Lectures :
...
L'unité d'`EntityOrm<T>` est une instruction. Quand deux écritures doivent toutes deux
