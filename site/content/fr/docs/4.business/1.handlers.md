...
Le container enregistre aussi un `EventBus` en singleton — un pub/sub in-process :

```ts
constructor(private eventBus: EventBus) {}

await this.eventBus.emit('post.published', { id });
```

Deux réserves. Il résout par **chaîne de caractères**, seul endroit du framework qui ne
résout pas par type — un nom d'event mal orthographié ne produit aucune erreur. Et il est
**in-process** : après une séparation, un émetteur et son auditeur qui ne partagent plus le
process ne se voient plus. Rien ne s'y abonne par défaut.
