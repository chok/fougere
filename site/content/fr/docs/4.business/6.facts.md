...
Tout autre appel nomme **un** destinataire : `fronds:` une adresse par Frond,
...
Un abonné est une opération ordinaire. Il garde sa porte, son validateur et ses middlewares, parce
...
`fronds:` est la seule ligne qui change. La Frond de l'auditeur reste sur le disque dans les
...
dans la donnée — parce que `fronds:` nomme une sorte de Frond, jamais une instance.

### Entre dépôts

Deux choses s'arrêtent à la frontière d'un dépôt, et `fronds:` n'en couvre qu'une.
...
// entrée — le même dispatch local, validateur et middlewares compris
...
arrive est validé contre la déclaration de l'émetteur lui-même.

Un porteur n'a pas d'adresse, et c'est la ligne entre les trois mots. `Emit` annonce un sujet
et `Fact` en reçoit un, donc aucun des deux ne nomme personne — un porteur peut les faire
traverser. Un `Pipe<T>` — l'op qui FINIT un fait — est APPELÉ : il a une adresse, locale ou
nommée dans
`fronds:`, et il répond. Un lien traverse donc un process et jamais un porteur, la même ligne
que `Ask` trace.
...
Un fait rencontre le **même validateur** que n'importe quelle entrée : une clé hors du contrat
