...
## `fougere verify` — l'hôte honore-t-il encore ce que vous avez accepté ?

```bash
fougere verify                              # chaque remote, à l'adresse d'où il a été synchronisé
fougere verify blog --from https://prod     # un remote, à une autre adresse
```

`sync` écrit `fougere.lock.json` à côté de votre config : une entrée par remote, portant
la carte **telle quelle**, comme elle a été reçue. Ce fichier est le contrat contre lequel
vous compilez, il vit dans votre dépôt, et son diff énonce en revue exactement ce que
l'autre côté a changé. `verify` le relit, demande sa carte à chaque hôte **maintenant**,
et rapporte ce qui ne tient plus :
...
**La comparaison se fait du côté du consommateur.** Ce que l'hôte a GAGNÉ ne bloque
jamais — une nouvelle porte, une nouvelle opération, un champ optionnel sont rapportés
pour que vous sachiez qu'un re-sync en vaut la peine. Ce sur quoi vous comptiez et que
vous n'avez plus est une rupture : un champ ou une opération disparus, un type qui a
bougé, un champ devenu requis, un fait qui n'est plus annoncé, ou une opération dont la
promesse a changé (`list` répondant une ligne là où vous déstructurez une page). Un
renommage n'est jamais deviné : la suppression est rapportée comme la rupture qu'elle
est, et la paire est nommée à côté.

Des bornes qui bougent comptent comme une rupture. `diff` dit qu'elles ont bougé sans
dire dans quel sens, et une carte est lue autant par des écrivains que par des lecteurs —
donc un élargissement apparaît comme une rupture qu'un re-sync efface, ce qui est le côté
pas cher de l'erreur.

**Les codes de sortie en font une barrière** : non-nul sur une rupture, zéro sinon. Un
hôte injoignable est rapporté et ne fait **pas** échouer — une panne n'est pas un contrat
rompu, et une vérification qui crie au loup cesse d'être lue.

`--from` est ce qui en fait une question d'avant-déploiement. Le contrat a été accepté
contre dev ; demandez à la staging avant de promouvoir, à la production avant d'expédier
le consommateur. Rien n'a été publié dans un registre et aucun déploiement n'a été
enregistré nulle part : l'état dont un broker garde une copie est lu sur ce qui le porte.

::callout
**Ce que ceci ne vérifie pas.** La carte porte des formes et des opérations, donc
`verify` répond sur la forme. Elle ne dit rien du comportement — qu'un post déjà publié
réponde 409 n'est dans aucune carte, et un test de contrat qui l'exerce garde sa place.
Il faut aussi un hôte qui réponde à `rpc.discover` ; contre un service qui n'est pas
Fougere, il n'y a rien à demander.
::
