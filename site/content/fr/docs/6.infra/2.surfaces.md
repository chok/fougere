...
`@fougere/schema-rest`, et cette porte s'y confronte au lieu d'en dériver une seconde. Un
...
les autres préfixes de lecture sont en `GET`, le reste en `POST`), et `operations: {
bySlug: { kind: 'query' } }` dans `frond.config.ts` l'énonce explicitement.
