...
connexion de stockage qu'on lui avait remise, ouverte avant que tout cela existe. Un provider qui tient quelque chose le dit par
`implements AsyncDisposable` : il y en a alors un par frond, et le scope de ce frond le ferme
en appelant `[Symbol.asyncDispose]()`, la méthode qu'appelle `await using`. Un provider qui ne
dit rien est construit pour chaque consommateur, et personne ne le ferme.
