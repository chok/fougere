...
peut partir derrière `fronds:`, alors qu'une extension appartient au **processus** qui les
...
connexion de stockage qu'on lui avait remise, ouverte avant que tout cela existe. Une classe construite par le
conteneur qui tient quelque chose le ferme dans `[Symbol.asyncDispose]()`, la méthode
qu'appelle `await using`, ou dans `[Symbol.dispose]()` : il y en a alors une par frond, et le scope de ce frond la ferme.
C'est la méthode qui est lue, à l'exécution : aucun `implements` n'est nécessaire, et la
réalisation d'un port compte comme n'importe quel provider. Une classe qui ne l'a pas est
construite pour chaque consommateur, et personne ne la ferme.
