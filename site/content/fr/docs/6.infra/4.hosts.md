...
`call`, `rest` et `session` prennent une `Request` et rendent une
...
app.use(call());             // ou une par une — l'enveloppe sert vos pages,
app.use('/admin', rest());   // REST est publique. Ce n'est pas une seule décision.
```
...
propre process avec `serve()`, et que les autres l'atteignent par `fronds:`. C'est le
