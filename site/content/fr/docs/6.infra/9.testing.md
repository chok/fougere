...
applique déjà à la topologie : `fronds:` est extérieur au frond, et le même code tourne
...
`applyCreate`/`applyUpdate` — qui lit l'horloge du système au moment d'écrire. Figer l'heure
comme dans n'importe quel projet suffit donc :
...
`toFake: ['Date']` laisse `setTimeout` et les promesses tourner : seules les dates sont figées.
...
## Ce qu'un appel a coûté

Un test de performance affirme un nombre de millisecondes et casse sur le portable d'un
autre. Ces deux-là posent une question de structure, et la structure ne change pas avec la
machine.

```ts
const ran = await statementsOf(() => facade.list());
expect(ran).toHaveLength(1);          // pas une par ligne
```

`statementsOf` rend toutes les requêtes que le bloc a lancées, dans l'ordre. `testApp` boote
sur du vrai SQLite : c'est donc ce que le handler a réellement demandé à la base — et une
requête par ligne est la forme d'un N+1. La façade remet au presenter la **page** entière
précisément pour qu'une recherche se fasse une seule fois ; rien ne refuse
`Promise.all(rows.map(…))` dans le corps du champ, et voilà ce qui s'en aperçoit.

Son dual lit l'appel lui-même :

```ts
const [op] = await spansOf(app, () => facade.checkout());
expect(op.selfMs).toBeLessThan(0.1 * op.ms);   // elle délègue, elle ne travaille pas
```

`selfMs` est la durée moins ce qui a couru dessous : l'assertion énonce donc qu'une op
délègue au lieu de travailler. Un ratio survit à une machine lente ; un seuil en
millisecondes teste la machine.

::note
`statementsOf` a besoin de `@fougere/adapter-sql` et `spansOf` de `@fougere/observability`.
Les deux **refusent** quand il manque, plutôt que de ne rien rendre : zéro est ce à quoi
ressemble une assertion qui passe, et un test qui ne peut pas échouer est pire que pas de
test.
::
