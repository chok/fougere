...
`inputFields` pour ce qui a le droit d'entrer, `Judge.onAbsent` pour ce qui est requis,
...
un second juge à tenir à la main.

`checkOutput` ne coûte rien à énoncer — `outputFields` y répond déjà, donc un champ que la
...
est à nous, c'est *quels* champs appartiennent à un corps — `inputFields`, le seul lecteur
...
Sur les faits, la ligne dit l'ordre à voix haute, parce qu'un fait est jugé strictement :
...
L'appartenance vient d'`inputFields`, l'obligation de l'axe `lifecycle`, et les bornes de
