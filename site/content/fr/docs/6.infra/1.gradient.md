...
| 2 | `remotes: { blog: '…' }` | la Frond tourne ailleurs, même code | le saut et son JSON ; le receveur lit l'identité sur le fil, donc le port est en loopback par défaut ; et deux capacités disparaissent — une [surface nommée](/fr/docs/infra/surfaces) ne répond rien pour une Frond distante, et une adresse par Frond veut dire que la même Frond ne peut pas être déployée deux fois |
