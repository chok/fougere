...
```
fronds/blog/
  vocabulary/           ← les mots que cette Frond ajoute à la déclaration — lu EN PREMIER
  ...
  middlewares/          ← ce qui entoure les opérations de cette Frond
  rules/                ← ce que le domaine calcule sans rien recevoir
  ...
  extensions/           ← ce qui monte et descend avec le process
  versions/             ← ce que ses formes étaient (`fougere freeze`)
```

Deux d'entre eux étendent le framework lui-même au lieu de déclarer un domaine.

`vocabulary/` tient les mots qu'une Frond ajoute à la déclaration — un générateur, un
format, un codec de frontière. Un fichier y enregistre au niveau du module, et c'est le
seul répertoire dont l'ORDRE compte : il est lu avant `entities/`, parce qu'une entité peut
écrire le nom qu'il vient d'ajouter, et qu'un registre refuse un nom qu'il ne tient pas.
...
// fronds/blog/entities/Post.ts — le mot est là quand cette ligne est lue
id: primary({ generate: 'ulid' }),
```

`extensions/` tient ce qui monte et descend avec le process, écrit à côté du code qu'il
instrumente. Un fichier y est reconnu à sa FORME — un module qui énonce `up` ou `down` — et
il voyage avec sa Frond : passée derrière `fronds:`, elle l'emmène sur le process qui la
sert là-bas. C'est pourquoi aucune clé de config ne l'élargit : ce qui doit être partout est
une Frond qui est partout, ce à quoi `Extension.fronds` répond déjà.
...
Le scanner reconnaît une Frond à ces dossiers, dans un dossier sous `fronds/`. Un deuxième
domaine s'ajoute à côté du premier, et le premier ne bouge pas.
...
export default defineFougere({
  conventions: {
    ...
    dirs: { entities: 'modeles' },            // les onze autres gardent leur nom
  },
})
```
...
aussi de clé aux enregistrements d'entités et à toute entrée `fronds:`.
...
d'entité, ses imports `@fronds/*` et ses entrées `fronds:`. Sans cette propriété, Fougere
