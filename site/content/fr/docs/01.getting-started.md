...
Le scanner lit sept dossiers de convention sous `fronds/` — `entities/`, `handlers/`,
`presenters/`, `collectors/`, `seeds/`, plus `services/` et `repositories/` qui sont deux
écritures d'une seule liste de providers. L'emplacement du fichier et le nom de la classe
servent à leur enregistrement ; aucun fichier de câblage supplémentaire n'est nécessaire.
...
`@fougere/nuxt` lit le scan et enregistre un alias `@fronds/<nom>` par Frond, donc cet
...
'@fronds/mon-shop/entities/Product'` — même règle d'alias, pas de segment `fronds/`, et le
...
export default defineFougere({
  ...
  //   false, ou pas de `db:` du tout    → lignes en mémoire, sur tous les hôtes, et le boot le dit
  db: { dialect: 'sqlite', path: '.data/app.db' },

  // Où vivent les lignes, quand elles ne vivent pas toutes dans `db`. Une entité
  // que ceci ne nomme pas reste dans la source par défaut : une seule base ne
  // déclare donc rien du tout.
  ...
  fronds: {
    ...
    // Auth (optionnel) — better-auth derrière une fine couche de traduction. C'est un frond
    // comme un autre : une entrée de `fronds:`, que `only:` laisse dehors là où le moteur ne
    // doit pas démarrer.
    auth: betterAuth({
...
une migration explicite. Les seeds tournent après cette synchronisation et écrivent leurs
lignes par le storage, qui les juge toujours.

Une application à plusieurs bases les déclare sous `sources:`, et une Frond qui doit
interroger à travers déclare `reads:` — voir [Les sources](/fr/docs/infra/sources).
