...
if (result.issues) {
  ...
  result.value;              // la ligne validée et décodée
}
```
...
entrante dans le cadre du la validation.
...
Le validateur décide si un `draft` absent est *légal*. Il l'est, donc l'entrée passe. Mais
...
envoyée et qu'on ne peut pas lui opposer. Valider et réaliser restent séparés : la même
entrée validée deux fois ne gagne jamais un champ en chemin.
...
| `lifecycle` — qui écrit, et quand | | le storage qui l'estampille |
| `boundary` — direction, conversions | en partie — le décodage entrant s'applique | `Visibility.input` / `Visibility.output` |

Un consommateur hors de Fougere obtient donc le validateur, dans le navigateur ou sur son
