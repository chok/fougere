...
Les deux sont duales. [`~standard`](/fr/docs/schema/standard-schema) remet le validateur
...
```ts
import { Bundle, Card } from '@fougere/schema';

const descriptor = Card.fromSchema(Post).descriptor;       // schéma → document JSON
const Rebuilt = Card.fromDescriptor(descriptor).toSchema(); // document → schéma vivant
```

`Card.fromSchema` produit le document canonique utilisé par les adapters. `Card.toSchema` rebâtit
...
nom porte le validateur et le type de ce qu'il validateur, exactement comme `class Post extends
entity({…}) {}`. C'est ce qu'écrit [`fougere sync`](/fr/docs/cli#fougere-sync--consommer-une-frond-distante) :

```ts
class Post extends Card.fromDescriptor<{ id: string; title: string }>(descriptor).toSchema() {}
```
...
### Une vue dit de quoi elle est la vue

La carte d'une [vue](/fr/docs/schema/views) porte une clé de plus, absente d'une
déclaration qui ne dérive de rien :

```jsonc
"x-fougere-derived": {
  "from": "Post",
  "survived": { "id": "id", "title": "title", "body": null, "status": null }
}
```

`from` nomme l'origine et `survived` dit ce que la coupe a laissé, indexé par les noms de
champs de l'origine — un champ retiré y figure à `null`. Le `title` portait déjà le nom de
l'origine, et le portait seul : deux vues d'une même entité se décrivaient à l'identique,
et rien dans la carte ne disait laquelle servait quels champs. Un lecteur peut désormais
répondre *un Post sans `body`*, et `fougere sync` écrire la dérivation plutôt qu'une forme
aplatie.
...
`Card.fromSchema` la résout depuis l'objet passé à `entity({...})`. Une contrainte composée comme
...
`Bundle.fromSchemas` produit un document `$defs` autonome. Chaque entité utilise son nom en
minuscules, qui correspond à la valeur de `relation.to`.

```ts
const bundle = Bundle.fromSchemas({ post: Post, author: Author });
const { post, author } = bundle.toSchemas();
```

Une relation est un pointeur et non un sous-schéma intégré : `Post → Author → Post` produit
deux références plutôt qu'une imbrication récursive. `Card.toSchema` conserve les noms ;
`Bundle.toSchemas` les résout vers les entités cibles attendues par les adapters.

## Limites de sérialisation

Une carte ne peut contenir que des données sérialisables :

- **une relation devient un nom.** `Bundle.toSchemas` peut résoudre le thunk `() => Author`
...
Héberger, c'est répondre : une entité sans façade est volontairement absente de `facades`,
sinon la carte publierait la forme de tables que personne ne peut atteindre — celles de
l'auth, typiquement.

Un [fait](/fr/docs/business/facts) est le cas inverse. Il ne porte pas d'opération non plus, mais quelqu'un a **écrit**
...
donne le contrat, `fronds:` ne donne que l'adresse, et entre deux dépôts il n'y a pas de
...
Le routeur de doublures ne lit que `facades`, et s'en sert pour associer les entités aux
adresses déclarées dans `fronds:`. Un fait n'est pas routable : personne ne l'appelle, il
arrive.
