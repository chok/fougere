 ...
 * local validator is the same one the handler runs — one declaration, both sides.
 *
  ...
  return (
    <main>
        ...
        {form.fields.map((field) => (
          <label key={field.name} style={{ display: 'grid', gap: '0.25rem' }}>
            ...
            {form.errors[field.name] && (
              <small style={{ color: '#b00' }}>{form.errors[field.name]}</small>
            )}
          </label>
        ))}
      ...
      <p style={{ color: '#999', marginTop: '2rem' }}>
        ...
        <code>publishedAt</code> are <code>readOnly</code> — none of the four is a client's to supply.
      </p>
    </main>
  );
}
