...
const { default: config } = await import(join(root, 'fougere.config.ts')) as {
  default: { sources?: Record<string, { source?: string; path: string; entities: string[] }> };
};
...
console.log(`\n${'─'.repeat(72)}`);
console.log(split
  ? 'sources: DECLARED — Ledger in its own database, RateCard in a directory of files'
...
await storageOf('rateCard').create({ code: 'EUR', rate: 0.5 });
